...
$handler->onBeginProcessData = function ($event) {
	...
	if ($connection == "layykien")
		$event->connectionString = REPORT_CONNECTION_STRING;
...
$handler->onSaveReport = function ($event) {
	...
	$fileName = $event->fileName; // Report file name
	$fileName = substr($fileName,0,strpos($fileName,"?n"));
	file_put_contents('report/'.$fileName.".mrt", $reportJson);
